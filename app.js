...
function bindTabs() {
  $("tabErrorBol")?.addEventListener("click", () => setActiveTab("error"));
  $("tabUnder500")?.addEventListener("click", () => setActiveTab("u500"));
  $("tabReport500")?.addEventListener("click", () => setActiveTab("report500"));
}

function setActiveTab(which) {
  $("tabErrorBol")?.classList.toggle("active", which === "error");
  $("tabUnder500")?.classList.toggle("active", which === "u500");
  $("tabReport500")?.classList.toggle("active", which === "report500");

  if (!AUTH.name) {
    $("loginCard")?.classList.remove("hidden");
    $("formCard")?.classList.add("hidden");
    $("under500Card")?.classList.add("hidden");
    $("report500Card")?.classList.add("hidden");
    return;
  }
  ...
  $("formCard")?.classList.toggle("hidden", which !== "error");
  $("under500Card")?.classList.toggle("hidden", which !== "u500");
  $("report500Card")?.classList.toggle("hidden", which !== "report500");

  if (which === "report500" && typeof initReport500Form === "function") {
    initReport500Form(window.AUTH || AUTH || {});
  }
}
...
async function onLogin() {
  ...
  AUTH = { name: lpsName, pass };
  window.AUTH = { ...(window.AUTH || {}), ...AUTH, userName: lpsName };
  setLpsFromLogin(lpsName);
  setActiveTab("error");
}
...
function openReport500() {
  setActiveTab("report500");
  if (typeof initReport500Form === "function") {
    initReport500Form(window.AUTH || AUTH || {});
  }
}
window.openReport500 = openReport500;
