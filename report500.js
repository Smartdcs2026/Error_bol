...
(function () {
  ...
  function getAuth() {
    return window.AUTH || state.auth || { name: "", pass: "" };
  }
  ...
  function renderSelect(id, list, withPlaceholder = true) {
    ...
    if (withPlaceholder) html.push(`<option value="">-- เลือก --</option>`);
    items.forEach((item) => {
      const v = typeof item === "object" ? norm(item.value || item.label || item.name) : norm(item);
      const t = typeof item === "object" ? norm(item.label || item.value || item.name) : norm(item);
      html.push(`<option value="${escapeHtml(v)}">${escapeHtml(t)}</option>`);
    });
    el.innerHTML = html.join("");
  }
  ...
  function ensureOptionValue(selectEl, value) {
    ...
    const v = norm(value);
    if (!v) return;
    const has = Array.from(selectEl.options || []).some((opt) => norm(opt.value) === v);
    if (has) return;
  ...
  async function checkReportRefDuplicate_(opts = {}) {
    ...
    if (!norm(refNo)) {
      resetReportRefDuplicateUi_();
      return { ok: true, duplicated: false, skipped: true, result: null };
    }

    const normalizedRef = String(refNo || "").trim();
    const url = `${apiUrl("/checkRefDuplicate")}?formType=${encodeURIComponent("report500")}&refNo=${encodeURIComponent(normalizedRef)}`;
    const res = await fetch(url, { method: "GET", cache: force ? "no-store" : "default" });
    const text = await res.text();
    let json = {};
    try { json = JSON.parse(text); } catch (_) { throw new Error("Backend ตอบกลับไม่ใช่ JSON"); }
    if (!res.ok || !json.ok) {
      throw new Error(json?.message || json?.error || `ตรวจสอบ Ref ไม่สำเร็จ (HTTP ${res.status})`);
    }
    if (json.duplicated) {
    ...
    return { ok: true, duplicated: !!json.duplicated, result: json };
  }
    ...
    await Swal.fire({
      ...
      html: `
        <div class="swalSummary" style="text-align:left">
          <div class="swalSection">
            <div class="swalSectionTitle">${escapeHtml(titleText)}</div>
            <div class="swalKvGrid">${commonTop}${detailHtml}</div>
          </div>
        </div>
      `,
      width: 920,
  ...
  function updateRptImagePreview(row, file, metaText) {
    ...
    const img = row.querySelector(".rptImagePreview");
    const empty = row.querySelector(".rptImagePreviewEmpty");
    if (meta) meta.textContent = metaText || buildRptEditedImageMeta(file, false);
    if (img) {
      if (img.dataset.objectUrl) {
        try { URL.revokeObjectURL(img.dataset.objectUrl); } catch (_) {}
      }
  ...
  function clearRptEditedImageState(row) {
    ...
    const empty = row?.querySelector(".rptImagePreviewEmpty");
    const meta = row?.querySelector(".rptImageMeta");
    if (fileInput) RPT_EDITED_IMAGE_STORE.delete(fileInput);
    if (img?.dataset.objectUrl) {
      try { URL.revokeObjectURL(img.dataset.objectUrl); } catch (_) {}
    }
    if (img) {
  ...
  function renderOptionMatrix(rootId, name, list) {
    ...
    root.innerHTML = items.map((item) => {
      const value = typeof item === "object" ? norm(item.value || item.label || item.name) : norm(item);
      const label = typeof item === "object" ? norm(item.label || item.value || item.name) : norm(item);
      return `
        <label class="optionChoice">
          <div class="optionChoiceCard">
    ...
    root.innerHTML = list.map((item, idx) => {
      ...
      return `
        <div class="rptWhereTypeRow" data-value="${escapeHtml(value)}" data-need-suffix="${needSuffix ? "1" : "0"}">
          <label class="optionChoice" for="${escapeHtml(rowId)}">
            <div class="optionChoiceCard">
              <input id="${escapeHtml(rowId)}" type="checkbox" class="rptWhereTypeChk" value="${escapeHtml(value)}">
              <span class="optionChoiceMark"></span>
              <span class="optionChoiceText">${escapeHtml(value)}</span>
            </div>
          </label>
          <div class="optionChoiceOther hidden">
            <input type="text" class="input rptWhereTypeSuffix" placeholder="กรอกข้อมูลต่อท้าย ${escapeHtml(value)}">
          </div>
        </div>
      `;
    }).join("");
  ...
  function expandOnlyThisCard(card) {
    ...
    if (!root) return;
    root.querySelectorAll(".rptRepeatCard").forEach((node) => {
      if (node === card) setCardCollapsed(node, false);
      else setCardCollapsed(node, true);
    });
  }
  ...
  function buildRepeatCardShell({ type, title, index, bodyHtml }) {
    return `
      <div class="rptRepeatCard is-empty" data-type="${escapeHtml(type)}">
        ...
        <div class="rptCardBody">${bodyHtml}</div>
      </div>
    `;
  }
  ...
  function getCardSummary(card) {
    ...
    if (type === "person") {
      ...
      const dep = isOther(depSel) ? (depOther || depSel) : depSel;
      const rem = isOther(remSel) ? (remOther || remSel) : remSel;
      const parts = [who ? `ชื่อ: ${who}` : "", pos ? `ตำแหน่ง: ${pos}` : "", dep ? `แผนก: ${dep}` : "", rem ? `หมายเหตุ: ${rem}` : ""].filter(Boolean);
      return parts.length ? cropText(parts.join(" • "), 120) : "ยังไม่กรอกข้อมูลผู้เกี่ยวข้อง";
    }
    if (type === "stepTaken") {
      const act = norm(card.querySelector(".rptStepActionType")?.value);
      ...
      const drugConfirm = norm(card.querySelector(".rptDrugConfirmed")?.value);
      const actionText = isOther(act) ? (actOther || act) : act;
      const extra = [alcoholResult ? `ผล: ${alcoholResult}` : "", alcoholMg ? `Mg%: ${alcoholMg}` : "", drugConfirm ? `ยืนยัน: ${drugConfirm}` : ""].filter(Boolean).join(" • ");
      const parts = [actionText ? `ประเภท: ${actionText}` : "", extra, detail ? `รายละเอียด: ${cropText(detail, 50)}` : ""].filter(Boolean);
      return parts.length ? cropText(parts.join(" • "), 120) : "ยังไม่กรอกข้อมูลการดำเนินการ";
    }
    if (type === "image") {
      const fileInput = card.querySelector(".rptImageFile");
      ...
      const file = edited || (fileInput?.files && fileInput.files[0]) || null;
      const caption = norm(card.querySelector(".rptImageCaption")?.value);
      const existingId = norm(card.getAttribute("data-existing-image-id"));
      const deleted = card.getAttribute("data-existing-deleted") === "1";
      const parts = [file ? "มีรูปภาพใหม่" : "", existingId && !deleted ? "มีรูปเดิม" : "", caption ? `คำบรรยาย: ${cropText(caption, 70)}` : ""].filter(Boolean);
      return parts.length ? parts.join(" • ") : "ยังไม่ได้เลือกรูปภาพ";
    }
    const idxTitle = norm(card.querySelector(".rptIdxTitle")?.value);
    const idxDetail = norm(card.querySelector(".rptIdxDetail")?.value);
  ...
  function evaluateCardState(card) {
    ...
    if (type === "person") {
      requiredSelectors = [".rptPersonWho"];
      allSelectors = [".rptPersonWho", ".rptPersonPosition", ".rptPersonPositionOther", ".rptPersonDepartment", ".rptPersonDepartmentOther", ".rptPersonRemark", ".rptPersonRemarkOther"];
    } else if (type === "stepTaken") {
      requiredSelectors = [".rptStepActionType", ".rptStepDetail"];
      allSelectors = [".rptStepActionType", ".rptStepActionTypeOther", ".rptAlcoholResult", ".rptAlcoholMgPercent", ".rptDrugConfirmed", ".rptDrugShortDetail", ".rptStepDetail"];
    } else if (type === "image") {
      const fileInput = card.querySelector(".rptImageFile");
      const edited = fileInput ? RPT_EDITED_IMAGE_STORE.get(fileInput)?.file : null;
      const raw = fileInput?.files && fileInput.files[0] ? fileInput.files[0] : null;
      const fileExists = !!(edited || raw);
      const existingId = norm(card.getAttribute("data-existing-image-id"));
      const existingDeleted = card.getAttribute("data-existing-deleted") === "1";
      const caption = norm(card.querySelector(".rptImageCaption")?.value);
      const effectiveExists = fileExists || (!!existingId && !existingDeleted);
      if (!effectiveExists && !caption) return { status: "empty", filled: 0, required: 1 };
      if (effectiveExists) return { status: "complete", filled: 1, required: 1 };
      return { status: "partial", filled: caption ? 1 : 0, required: 1 };
    } else {
      requiredSelectors = [".rptIdxTitle", ".rptIdxDetail"];
      allSelectors = [".rptIdxTitle", ".rptIdxDetail"];
  ...
  function refreshSingleCardUi(card) {
    ...
    const stateInfo = evaluateCardState(card);
    card.classList.remove("is-empty", "is-partial", "is-complete");
    card.classList.add(stateInfo.status === "complete" ? "is-complete" : stateInfo.status === "partial" ? "is-partial" : "is-empty");
    const summary = card.querySelector(".rptCardSummary");
    if (summary) summary.textContent = getCardSummary(card);
    const badge = card.querySelector(".rptCardStatus");
    if (badge) {
      badge.textContent = stateInfo.status === "complete" ? "กรอกแล้ว" : stateInfo.status === "partial" ? "กรอกบางส่วน" : "ยังไม่กรอก";
    }
    const head = card.querySelector(".rptCardHead");
    if (head) head.setAttribute("aria-expanded", card.classList.contains("is-collapsed") ? "false" : "true");
  }
  ...
  function ensureRepeatFooterButton(listId) {
    ...
    if (!footer) {
      ...
      footer.setAttribute("data-target", listId);
      footer.innerHTML = `
        <button type="button" class="btn ghost rptRepeatAddBottomBtn">+ เพิ่ม${escapeHtml(label)}</button>
      `;
      root.insertAdjacentElement("afterend", footer);
      footer.querySelector("button")?.addEventListener("click", () => {
        $(addBtnId)?.click();
  ...
  function createPersonRowHtml(index) {
    ...
    return buildRepeatCardShell({
      ...
      bodyHtml: `
        <div class="gridCompact">
          <div class="field"><label>ผู้เกี่ยวข้อง (Who is involved)</label><input type="text" class="rptPersonWho" placeholder="ชื่อผู้เกี่ยวข้อง"></div>
          <div class="field"><label>Position</label><select class="rptPersonPosition">${positionOptions}</select></div>
          <div class="field rptPersonPositionOtherWrap hidden"><label>Position อื่นๆ</label><input type="text" class="rptPersonPositionOther" placeholder="ระบุตำแหน่งเพิ่มเติม"></div>
          <div class="field"><label>Department</label><select class="rptPersonDepartment">${departmentOptions}</select></div>
          <div class="field rptPersonDepartmentOtherWrap hidden"><label>Department อื่นๆ</label><input type="text" class="rptPersonDepartmentOther" placeholder="ระบุส่วนงานเพิ่มเติม"></div>
          <div class="field"><label>Remark</label><select class="rptPersonRemark">${remarkOptions}</select></div>
          <div class="field rptPersonRemarkOtherWrap hidden"><label>Remark อื่นๆ</label><input type="text" class="rptPersonRemarkOther" placeholder="ระบุหมายเหตุเพิ่มเติม"></div>
        </div>
        <div class="rptCardFooterActions">
          <button type="button" class="btn ghost rptCollapseRow">ย่อการ์ด</button>
          <button type="button" class="btn ghost rptRemoveRow">ลบแถว</button>
  ...
  function createStepTakenRowHtml(index) {
    ...
    return buildRepeatCardShell({
      ...
      bodyHtml: `
        <div class="gridCompact">
          <div class="field"><label>ประเภทการดำเนินการ</label><select class="rptStepActionType">${actionOptions}</select></div>
          <div class="field rptStepActionTypeOtherWrap hidden"><label>ระบุการดำเนินการอื่นๆ</label><input type="text" class="rptStepActionTypeOther" placeholder="ระบุเอง"></div>
          <div class="field rptAlcoholResultWrap hidden"><label>ผลการตรวจแอลกอฮอล์</label><select class="rptAlcoholResult"><option value="">-- เลือก --</option><option value="พบ">พบ</option><option value="ไม่พบ">ไม่พบ</option></select></div>
          <div class="field rptAlcoholMgWrap hidden"><label>ปริมาณแอลกอฮอล์ (Mg%)</label><input type="number" class="rptAlcoholMgPercent" placeholder="กรอกเฉพาะตัวเลข" inputmode="decimal"></div>
          <div class="field rptDrugConfirmedWrap hidden"><label>ผลยืนยันการเสพ</label><input type="text" class="rptDrugConfirmed" placeholder="เช่น ยืนยันผลบวก"></div>
          <div class="field rptDrugDetailWrap hidden"><label>รายละเอียดแบบย่อ</label><textarea class="rptDrugShortDetail" rows="3" placeholder="รายละเอียดการตรวจสารเสพติด"></textarea></div>
          <div class="field fieldSpan2"><label>รายละเอียดเพิ่มเติม</label><textarea class="rptStepDetail" rows="3" placeholder="รายละเอียดเพิ่มเติมของการดำเนินการ"></textarea></div>
        </div>
        <div class="rptCardFooterActions">
          <button type="button" class="btn ghost rptCollapseRow">ย่อการ์ด</button>
          <button type="button" class="btn ghost rptRemoveRow">ลบแถว</button>
  ...
  function createImageRowHtml(index) {
    return buildRepeatCardShell({
      ...
      bodyHtml: `
        ...
        <div class="field" style="margin-top:10px">
          <div class="rptImagePreviewEmpty" style="padding:12px 14px;border:1px dashed #bfd1e6;border-radius:14px;background:#f8fbff;color:#64748b;font-size:12px;font-weight:800">ยังไม่ได้เลือกรูปภาพ</div>
          <img class="rptImagePreview hidden" alt="preview" style="width:100%;max-height:260px;object-fit:contain;border-radius:14px;border:1px solid #d9e4f1;background:#fff;padding:6px">
        </div>
        <div class="rptCardFooterActions rptImageActions">
          <button type="button" class="btn ghost rptCollapseRow">ย่อการ์ด</button>
          <button type="button" class="btn ghost rptUseExistingImageBtn hidden">คงรูปเดิม</button>
          <button type="button" class="btn danger rptDeleteExistingImageBtn hidden">ลบรูปเดิม</button>
          <button type="button" class="btn ghost rptEditImageBtn">แก้ไขภาพ</button>
          <button type="button" class="btn ghost rptRemoveRow">ลบแถว</button>
        </div>
      `
  ...
  function updateExistingImageButtons(node) {
    const useBtn = node.querySelector(".rptUseExistingImageBtn");
    const delBtn = node.querySelector(".rptDeleteExistingImageBtn");
    const existingId = norm(node.getAttribute("data-existing-image-id"));
    const deleted = node.getAttribute("data-existing-deleted") === "1";
    if (!existingId) {
    ...
    if (useBtn) useBtn.textContent = deleted ? "คืนรูปเดิม" : "คงรูปเดิม";
    if (delBtn) delBtn.textContent = deleted ? "ลบแล้ว" : "ลบรูปเดิม";
    delBtn?.classList.toggle("secondary", deleted);
    delBtn?.classList.toggle("danger", !deleted);
  }

  function setExistingImageDeleted(node, deleted) {
    if (!node) return;
    node.setAttribute("data-existing-deleted", deleted ? "1" : "0");
    const existingId = norm(node.getAttribute("data-existing-image-id"));
    if (existingId) {
      const idx = state.editMode.removedImageIds.indexOf(existingId);
      if (deleted && idx < 0) state.editMode.removedImageIds.push(existingId);
      if (!deleted && idx >= 0) state.editMode.removedImageIds.splice(idx, 1);
    }
    updateExistingImageButtons(node);
    refreshSingleCardUi(node);
  }

  function applyExistingImagePreview(node, imgMeta) {
    if (!node || !imgMeta) return;
    const img = node.querySelector(".rptImagePreview");
    const empty = node.querySelector(".rptImagePreviewEmpty");
    const meta = node.querySelector(".rptImageMeta");
    const url = norm(imgMeta.url || imgMeta.viewUrl);
    node.setAttribute("data-existing-image-id", norm(imgMeta.id));
    node.setAttribute("data-existing-deleted", "0");
    if (url && img) {
      img.src = url;
      img.classList.remove("hidden");
    }
    empty?.classList.add("hidden");
    if (meta) meta.textContent = `ไฟล์เดิม: ${norm(imgMeta.filename || imgMeta.name || "image.jpg")}`;
    if (node.querySelector(".rptImageCaption") && norm(imgMeta.caption)) {
      node.querySelector(".rptImageCaption").value = norm(imgMeta.caption);
    }
    updateExistingImageButtons(node);
    refreshSingleCardUi(node);
  }
  ...
  function bindImageRow(node) {
    const fileInput = node.querySelector(".rptImageFile");
    const editBtn = node.querySelector(".rptEditImageBtn");
    const useExistingBtn = node.querySelector(".rptUseExistingImageBtn");
    const delExistingBtn = node.querySelector(".rptDeleteExistingImageBtn");
    if (!fileInput) return;
    ...
    fileInput.addEventListener("change", async () => {
      ...
      if (!file) {
        const existingId = norm(node.getAttribute("data-existing-image-id"));
        if (!existingId || node.getAttribute("data-existing-deleted") === "1") {
          clearRptEditedImageState(node);
        }
        refreshSingleCardUi(node);
        return;
      ...
      if (!/^image\//i.test(file.type || "")) {
        fileInput.value = "";
        clearRptEditedImageState(node);
        await Swal.fire({ icon: "warning", title: "ไฟล์ไม่ถูกต้อง", text: "กรุณาเลือกไฟล์รูปภาพเท่านั้น" });
        return;
      }
      RPT_EDITED_IMAGE_STORE.delete(fileInput);
      updateRptImagePreview(node, file, buildRptEditedImageMeta(file, false));
    ...
    useExistingBtn?.addEventListener("click", () => {
      setExistingImageDeleted(node, false);
      const existingId = norm(node.getAttribute("data-existing-image-id"));
      if (existingId && fileInput) {
        fileInput.value = "";
        RPT_EDITED_IMAGE_STORE.delete(fileInput);
      }
      refreshSingleCardUi(node);
    ...
    delExistingBtn?.addEventListener("click", () => {
      const deleted = node.getAttribute("data-existing-deleted") === "1";
      setExistingImageDeleted(node, !deleted);
    });
  }
  ...
  function bindDynamicRow(node) {
    ...
    const toggleCard = () => {
      const collapsed = node.classList.contains("is-collapsed");
      if (collapsed) expandOnlyThisCard(node);
      else setCardCollapsed(node, true);
      refreshSingleCardUi(node);
    };
    ...
    node.querySelector(".rptRemoveRow")?.addEventListener("click", () => {
      ...
      const cfg = getRepeatConfig(listId);
      const existingId = norm(node.getAttribute("data-existing-image-id"));
      if (existingId) setExistingImageDeleted(node, true);
      node.remove();
      refreshRowIndex(listId);
      toggleEmptyState(listId, cfg.label);
    });
  ...
  function collectPersons() {
    return Array.from(document.querySelectorAll("#rptPersonList .rptRepeatCard")).map((card, idx) => ({
      ...
      remark: norm(card.querySelector(".rptPersonRemark")?.value),
      remarkOther: norm(card.querySelector(".rptPersonRemarkOther")?.value)
    })).filter((x) => x.who || x.position || x.positionOther || x.department || x.departmentOther || x.remark || x.remarkOther);
  }
  ...
  function collectStepTakens() {
    return Array.from(document.querySelectorAll("#rptStepTakenList .rptRepeatCard")).map((card, idx) => ({
      ...
      drugShortDetail: norm(card.querySelector(".rptDrugShortDetail")?.value),
      detail: norm(card.querySelector(".rptStepDetail")?.value)
    })).filter((x) => x.actionType || x.actionTypeOther || x.alcoholResult || x.alcoholMgPercent || x.drugConfirmed || x.drugShortDetail || x.detail);
  }
  ...
  async function collectImages() {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const file = edited || raw;
      const caption = norm(row.querySelector(".rptImageCaption")?.value);
      const existingId = norm(row.getAttribute("data-existing-image-id"));
      const existingDeleted = row.getAttribute("data-existing-deleted") === "1";

      if (!file && !caption && !existingId) continue;

      if (!file) {
        if (existingId && !existingDeleted) {
          out.push({ existingId, keep: true, caption });
          continue;
        }
        if (existingId && existingDeleted) {
          out.push({ existingId, keep: false, deleted: true, caption });
          continue;
        }
  ...
  function collectEmailRecipients() {
    const checked = Array.from(document.querySelectorAll(".rptEmailChk:checked")).map((el) => norm(el.value)).filter(Boolean);
    const extra = splitMultiEmails($("rptEmailOther")?.value || "");
    return uniqueEmails([].concat(checked).concat(extra));
  }
  ...
  async function searchDisciplineByEmployeeCode(employeeCode) {
    ...
    const res = await fetch(apiUrl(`/disciplineLookup?employeeCode=${encodeURIComponent(code)}`), { method: "GET" });
    const raw = await res.text();
    let json = null;
    try { json = JSON.parse(raw); } catch (_) { throw new Error(`ระบบค้นหาวินัยไม่ส่ง JSON กลับมา (HTTP ${res.status})`); }
    if (!res.ok || !json || !json.ok) {
      throw new Error(json?.error || `ค้นหาการดำเนินการทางวินัยไม่สำเร็จ (HTTP ${res.status})`);
    }
    return json;
  ...
  async function searchItemLookup(item) {
    const clean = norm(item);
    if (!clean) throw new Error("กรุณาระบุ Item");
    const res = await fetch(apiUrl(`/itemLookup?item=${encodeURIComponent(clean)}`), { method: "GET" });
    let json = null;
    try { json = await res.json(); } catch (_) { throw new Error("ระบบค้นหา Item ไม่ส่ง JSON กลับมา"); }
    if (!res.ok || !json || !json.ok) {
      throw new Error(json?.error || "ค้นหารายการสินค้าไม่สำเร็จ");
    }
    state.itemLookup = {
  ...
  function renderDisciplineLookupTable(records, meta = {}) {
    ...
    const employeeName = escapeHtml(meta.employeeName || "");
    if (!rows.length) {
      return `<div class="rptLookupEmpty">ไม่พบข้อมูลการดำเนินการทางวินัย</div>`;
    }
    ...
    const cardBody = rows.map((row, idx) => `
      <div class="rptLookupRecordCard rptLookupRecordCardCompact">
        ...
        <div class="rptLookupRecordGrid rptLookupRecordGridCompact">
          <div class="rptLookupRecordItem rptLookupRecordItemWide"><div class="rptLookupRecordLabel">เรื่อง</div><div class="rptLookupRecordValue">${escapeHtml(row.subject || "-")}</div></div>
          <div class="rptLookupRecordItem"><div class="rptLookupRecordLabel">สถานะเอกสาร</div><div class="rptLookupRecordValue">${escapeHtml(row.docStatus || "-")}</div></div>
          <div class="rptLookupRecordItem"><div class="rptLookupRecordLabel">ผลการดำเนินการ</div><div class="rptLookupRecordValue">${escapeHtml(row.result || "-")}</div></div>
          <div class="rptLookupRecordItem"><div class="rptLookupRecordLabel">ผู้บังคับบัญชา</div><div class="rptLookupRecordValue">${escapeHtml(row.supervisor || "-")}</div></div>
          <div class="rptLookupRecordItem"><div class="rptLookupRecordLabel">วันที่ดำเนินการลงโทษ</div><div class="rptLookupRecordValue">${escapeHtml(row.actionDate || "-")}</div></div>
        </div>
      </div>
    `).join("");
    ...
    return `
      ...
      <div class="rptLookupResultCard rptLookupResultCardDiscipline">
        ...
        <div class="rptLookupMobileView"><div class="rptLookupRecordList rptLookupRecordListCompact">${cardBody}</div></div>
      </div>
    `;
  }
  ...
  function renderItemLookupResult(result) {
    ...
    return `
      ...
      <div class="rptLookupResultCard">
        <div class="rptLookupResultGrid rptLookupResultGridCompact">
          <div class="rptLookupResultBox"><div class="rptLookupResultLabel">รหัสสินค้า</div><div class="rptLookupResultValue">${item || "-"}</div></div>
          <div class="rptLookupResultBox"><div class="rptLookupResultLabel">ชื่อสินค้า</div><div class="rptLookupResultValue">${description || "-"}</div></div>
        </div>
        <div class="rptLookupResultTextBlock">
          <div class="rptLookupResultTextTitle">ข้อความพร้อมใช้งาน</div>
          <div class="rptLookupResultTextValue">${displayText || "-"}</div>
  ...
  function attachDisciplineLookupResult(result) {
    ...
    const normalizedEmployeeCode = norm(result?.normalizedEmployeeCode || "");
    const matchCount = Number(result?.count || records.length || 0);
    state.disciplineLookup = { employeeCode, employeeName, normalizedEmployeeCode, matchCount, records, attached: true, searched: true };
    updateDisciplineSummaryCard();
  }
  ...
  function copyTextToClipboard(text, successMessage) {
    const value = String(text || "").trim();
    if (!value) {
      return Swal.fire({ icon: "warning", title: "ไม่มีข้อมูลให้คัดลอก", text: "กรุณาค้นหา Item ก่อน" });
    }
    const done = () => Swal.fire({ icon: "success", title: "คัดลอกแล้ว", text: successMessage || "คัดลอกข้อมูลเรียบร้อย", timer: 1200, showConfirmButton: false });
    if (navigator.clipboard && navigator.clipboard.writeText) {
      return navigator.clipboard.writeText(value).then(done).catch(() => fallbackCopy(value, done));
    }
    return fallbackCopy(value, done);
  ...
  function insertItemTextToField(fieldId, text, mode = "replace") {
    ...
    const value = String(text || "").trim();
    if (!el) {
      return Swal.fire({ icon: "warning", title: "ไม่พบช่องข้อมูล", text: `ไม่พบฟิลด์ ${fieldId}` });
    }
    if (!value) {
      return Swal.fire({ icon: "warning", title: "ไม่มีข้อมูลให้ใส่", text: "กรุณาค้นหา Item ก่อน" });
    }
    if (mode === "append" && norm(el.value)) {
      el.value = `${norm(el.value)} ${value}`;
    } else {
    ...
    el.dispatchEvent(new Event("input", { bubbles: true }));
    el.dispatchEvent(new Event("change", { bubbles: true }));
    return Swal.fire({ icon: "success", title: "ใส่ข้อมูลแล้ว", text: "นำข้อมูลสินค้าไปใส่ในรายงานเรียบร้อย", timer: 1200, showConfirmButton: false });
  }
    ...
    await Swal.fire({
      ...
      html: `
        <div class="rptLookupModal">
          ...
          <div class="rptLookupBody">
            <div id="swalRptDisciplineResult" class="rptLookupResult">
              <div class="rptLookupState"><div class="rptLookupStateInner"><div class="rptLookupStateIcon">⌕</div><div class="rptLookupStateTitle">พร้อมค้นหาข้อมูลวินัย</div><div class="rptLookupStateText">กรอกรหัสพนักงานแล้วกดค้นหา ระบบจะแสดงประวัติการดำเนินการทางวินัยในพื้นที่ด้านล่าง</div></div></div>
            </div>
          </div>
        </div>
      `,
      didOpen: () => {
        ...
        const runSearch = async () => {
          const employeeCode = norm(input?.value || "");
          resultBox.innerHTML = `<div class="rptLookupState"><div class="rptLookupStateInner"><div class="rptLookupStateIcon">…</div><div class="rptLookupStateTitle">กำลังค้นหา</div><div class="rptLookupStateText">ระบบกำลังค้นหาประวัติการดำเนินการทางวินัย โปรดรอสักครู่</div></div></div>`;
          btnUse.disabled = true;
          latestResult = null;
          try {
            const json = await searchDisciplineByEmployeeCode(employeeCode);
            latestResult = json;
            const count = Number(json.count || (json.records || []).length || 0);
            resultBox.innerHTML = renderDisciplineLookupTable(json.records || [], { count, employeeCode: json.employeeCode || json.normalizedEmployeeCode || "", employeeName: json.employeeName || "" });
            btnUse.disabled = !(json.records && json.records.length);
          } catch (err) {
            resultBox.innerHTML = `<div class="rptLookupEmpty">${escapeHtml(err.message || String(err))}</div>`;
          }
      ...
      html: `
        <div class="rptLookupModal">
          <div class="rptLookupModalHead">
            <div class="rptLookupModalBadge">ITEM LOOKUP</div>
            <div class="rptLookupModalTitle">ค้นหารายการสินค้า</div>
            <div class="rptLookupModalSub">ค้นหาชื่อสินค้า และนำข้อความไปใส่ในช่องเรื่องหรือเหตุที่เกิดได้ทันที</div>
          </div>
          <div class="rptLookupToolbar rptLookupToolbarCompact">
            <div class="field">
              <label for="swalRptItemCode">Item</label>
              <input id="swalRptItemCode" class="rptLookupInput" value="${escapeHtml(initialItem)}" placeholder="กรอก Item">
            </div>
            <div class="rptLookupToolbarActions rptLookupToolbarActionsCompact">
              <button type="button" id="swalRptItemSearch" class="rptLookupBtn primary">ค้นหา</button>
            </div>
          ...
          <div class="rptLookupBody">
            <div id="swalRptItemResult" class="rptLookupResult">${renderItemLookupResult(null)}</div>
          </div>
        </div>
      `,
      didOpen: () => {
        ...
        const runSearch = async () => {
          const item = norm(input?.value || "");
          resultBox.innerHTML = `<div class="rptLookupState"><div class="rptLookupStateInner"><div class="rptLookupStateIcon">…</div><div class="rptLookupStateTitle">กำลังค้นหา</div><div class="rptLookupStateText">กำลังค้นหารายการสินค้า โปรดรอสักครู่</div></div></div>`;
          try {
            latestResult = await searchItemLookup(item);
            resultBox.innerHTML = renderItemLookupResult(latestResult);
            document.getElementById("swalRptItemCopyDesc")?.addEventListener("click", () => copyTextToClipboard(latestResult?.description || "", "คัดลอกชื่อสินค้าเรียบร้อย"));
            document.getElementById("swalRptItemCopyFull")?.addEventListener("click", () => copyTextToClipboard(latestResult?.displayText || "", "คัดลอกข้อความเต็มเรียบร้อย"));
            document.getElementById("swalRptItemToSubject")?.addEventListener("click", () => insertItemTextToField("rptSubject", latestResult?.displayText || latestResult?.description || "", "replace"));
            document.getElementById("swalRptItemToWhat")?.addEventListener("click", () => insertItemTextToField("rptWhatHappen", latestResult?.displayText || latestResult?.description || "", "append"));
          } catch (err) {
            resultBox.innerHTML = `<div class="rptLookupEmpty">${escapeHtml(err.message || String(err))}</div>`;
          }
        };
  ...
  function addDefaultRepeatRows() {
    if (!document.querySelector("#rptPersonList .rptRepeatCard")) appendRow("rptPersonList", createPersonRowHtml(1), "ผู้เกี่ยวข้อง");
    if (!document.querySelector("#rptDamageList .rptRepeatCard")) appendRow("rptDamageList", createSimpleIndexedRowHtml("damage", 1, "ความเสียหาย", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "ความเสียหาย");
    if (!document.querySelector("#rptStepTakenList .rptRepeatCard")) appendRow("rptStepTakenList", createStepTakenRowHtml(1), "การดำเนินการ");
    if (!document.querySelector("#rptEvidenceList .rptRepeatCard")) appendRow("rptEvidenceList", createSimpleIndexedRowHtml("evidence", 1, "หลักฐาน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "หลักฐาน");
    if (!document.querySelector("#rptCauseList .rptRepeatCard")) appendRow("rptCauseList", createSimpleIndexedRowHtml("cause", 1, "สาเหตุ", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "สาเหตุ");
    if (!document.querySelector("#rptPreventionList .rptRepeatCard")) appendRow("rptPreventionList", createSimpleIndexedRowHtml("prevention", 1, "การป้องกัน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "การป้องกัน");
    if (!document.querySelector("#rptLearningList .rptRepeatCard")) appendRow("rptLearningList", createSimpleIndexedRowHtml("learning", 1, "ข้อสรุป/บทเรียน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "ข้อสรุป/บทเรียน");
    if (!document.querySelector("#rptImageList .rptRepeatCard")) appendRow("rptImageList", createImageRowHtml(1), "รูปภาพ");
  }

  function normalizeReport500OptionsResponse(json) {
    const src = (json && typeof json === "object") ? ((json.data && typeof json.data === "object") ? json.data : json) : {};
    const toArray = (v) => Array.isArray(v) ? v : [];
    return {
      branchList: toArray(src.branchList),
      reportTypeList: toArray(src.reportTypeList),
  ...
  function bindTopButtons() {
    ...
    $("btnRptLoadForEdit")?.addEventListener("click", () => {
      promptLoadReport500ForEdit_().catch((err) => {
        Swal.fire({ icon: "error", title: "โหลดข้อมูลไม่สำเร็จ", text: err?.message || String(err) });
      });
    });

    $("btnRptAddPerson")?.addEventListener("click", () => appendRow("rptPersonList", createPersonRowHtml(document.querySelectorAll("#rptPersonList .rptRepeatCard").length + 1), "ผู้เกี่ยวข้อง"));
    $("btnRptAddDamage")?.addEventListener("click", () => appendRow("rptDamageList", createSimpleIndexedRowHtml("damage", document.querySelectorAll("#rptDamageList .rptRepeatCard").length + 1, "ความเสียหาย", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "ความเสียหาย"));
    $("btnRptAddStepTaken")?.addEventListener("click", () => appendRow("rptStepTakenList", createStepTakenRowHtml(document.querySelectorAll("#rptStepTakenList .rptRepeatCard").length + 1), "การดำเนินการ"));
    $("btnRptAddEvidence")?.addEventListener("click", () => appendRow("rptEvidenceList", createSimpleIndexedRowHtml("evidence", document.querySelectorAll("#rptEvidenceList .rptRepeatCard").length + 1, "หลักฐาน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "หลักฐาน"));
    $("btnRptAddCause")?.addEventListener("click", () => appendRow("rptCauseList", createSimpleIndexedRowHtml("cause", document.querySelectorAll("#rptCauseList .rptRepeatCard").length + 1, "สาเหตุ", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "สาเหตุ"));
    $("btnRptAddPrevention")?.addEventListener("click", () => appendRow("rptPreventionList", createSimpleIndexedRowHtml("prevention", document.querySelectorAll("#rptPreventionList .rptRepeatCard").length + 1, "การป้องกัน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "การป้องกัน"));
    $("btnRptAddLearning")?.addEventListener("click", () => appendRow("rptLearningList", createSimpleIndexedRowHtml("learning", document.querySelectorAll("#rptLearningList .rptRepeatCard").length + 1, "ข้อสรุป/บทเรียน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด"), "ข้อสรุป/บทเรียน"));
    $("btnRptAddImage")?.addEventListener("click", () => appendRow("rptImageList", createImageRowHtml(document.querySelectorAll("#rptImageList .rptRepeatCard").length + 1), "รูปภาพ"));
  }
  ...
  function bindLookupButtons() {
    ...
    state.lookupButtonsBound = true;
    $("btnRptDisciplineLookup")?.addEventListener("click", openDisciplineLookupPopup);
    $("btnRptDisciplinePreview")?.addEventListener("click", async () => {
      const d = state.disciplineLookup;
      if (!d.attached || !d.records.length) {
        await Swal.fire({ icon: "info", title: "ยังไม่มีข้อมูลวินัย", text: "กรุณาค้นหาและแนบข้อมูลวินัยก่อน" });
        return;
      }
      await Swal.fire({
        customClass: { popup: "rptLookupPreviewPopup", title: "rptLookupPreviewTitle", htmlContainer: "rptLookupPreviewHtml" },
        title: "ตัวอย่างข้อมูลวินัยที่แนบ",
        html: `<div class="rptLookupPreviewWrap">${renderDisciplineLookupTable(d.records, { count: d.matchCount, employeeCode: d.employeeCode, employeeName: d.employeeName })}</div>`,
        confirmButtonText: "ปิด"
      });
    });
    $("btnRptClearDiscipline")?.addEventListener("click", () => {
      resetDisciplineLookupState();
    });
    $("btnRptItemLookup")?.addEventListener("click", openItemLookupPopup);
  }

  function setChecksByName(name, entries) {
    const map = new Map((Array.isArray(entries) ? entries : []).map((x) => [norm(x.value), !!x.checked]));
    document.querySelectorAll(`input[name="${name}"]`).forEach((el) => {
      el.checked = map.get(norm(el.value)) || false;
    });
  }

  function setWhereTypeSelections(entries) {
    const map = new Map((Array.isArray(entries) ? entries : []).map((x) => [norm(x.value), x]));
    document.querySelectorAll(".rptWhereTypeRow").forEach((row) => {
      const chk = row.querySelector(".rptWhereTypeChk");
      const suffix = row.querySelector(".rptWhereTypeSuffix");
      const entry = map.get(norm(chk?.value));
      chk.checked = !!entry?.checked;
      if (suffix) suffix.value = norm(entry?.suffixText);
      chk.dispatchEvent(new Event("change", { bubbles: true }));
    });
  }

  function setEmailSelections(emails, emailOther) {
    const set = new Set(uniqueEmails(emails).map((x) => x.toLowerCase()));
    document.querySelectorAll(".rptEmailChk").forEach((el) => {
      el.checked = set.has(norm(el.value).toLowerCase());
    });
    if ($("rptEmailOther")) $("rptEmailOther").value = norm(emailOther);
  }

  function fillIndexedRows(listId, rows, type, titleLabel, detailLabel, titlePlaceholder, detailPlaceholder) {
    const root = $(listId);
    if (!root) return;
    root.innerHTML = "";
    const arr = Array.isArray(rows) && rows.length ? rows : [{ title: "", detail: "" }];
    arr.forEach((item, idx) => {
      const node = appendRow(listId, createSimpleIndexedRowHtml(type, idx + 1, titleLabel, detailLabel, titlePlaceholder, detailPlaceholder), getRepeatConfig(listId).label);
      node.querySelector(".rptIdxTitle").value = norm(item.title);
      node.querySelector(".rptIdxDetail").value = norm(item.detail);
      refreshSingleCardUi(node);
    });
  }

  function fillPersons(rows) {
    const listId = "rptPersonList";
    const root = $(listId);
    if (!root) return;
    root.innerHTML = "";
    const arr = Array.isArray(rows) && rows.length ? rows : [{}];
    arr.forEach((item, idx) => {
      const node = appendRow(listId, createPersonRowHtml(idx + 1), getRepeatConfig(listId).label);
      node.querySelector(".rptPersonWho").value = norm(item.who);
      ensureOptionValue(node.querySelector(".rptPersonPosition"), norm(item.position));
      node.querySelector(".rptPersonPosition").value = norm(item.position);
      node.querySelector(".rptPersonPositionOther").value = norm(item.positionOther);
      ensureOptionValue(node.querySelector(".rptPersonDepartment"), norm(item.department));
      node.querySelector(".rptPersonDepartment").value = norm(item.department);
      node.querySelector(".rptPersonDepartmentOther").value = norm(item.departmentOther);
      ensureOptionValue(node.querySelector(".rptPersonRemark"), norm(item.remark));
      node.querySelector(".rptPersonRemark").value = norm(item.remark);
      node.querySelector(".rptPersonRemarkOther").value = norm(item.remarkOther);
      node.querySelector(".rptPersonPosition")?.dispatchEvent(new Event("change", { bubbles: true }));
      node.querySelector(".rptPersonDepartment")?.dispatchEvent(new Event("change", { bubbles: true }));
      node.querySelector(".rptPersonRemark")?.dispatchEvent(new Event("change", { bubbles: true }));
      refreshSingleCardUi(node);
    });
  }

  function fillStepTakens(rows) {
    const listId = "rptStepTakenList";
    const root = $(listId);
    if (!root) return;
    root.innerHTML = "";
    const arr = Array.isArray(rows) && rows.length ? rows : [{}];
    arr.forEach((item, idx) => {
      const node = appendRow(listId, createStepTakenRowHtml(idx + 1), getRepeatConfig(listId).label);
      ensureOptionValue(node.querySelector(".rptStepActionType"), norm(item.actionType));
      node.querySelector(".rptStepActionType").value = norm(item.actionType);
      node.querySelector(".rptStepActionTypeOther").value = norm(item.actionTypeOther);
      node.querySelector(".rptAlcoholResult").value = norm(item.alcoholResult);
      node.querySelector(".rptAlcoholMgPercent").value = norm(item.alcoholMgPercent);
      node.querySelector(".rptDrugConfirmed").value = norm(item.drugConfirmed);
      node.querySelector(".rptDrugShortDetail").value = norm(item.drugShortDetail);
      node.querySelector(".rptStepDetail").value = norm(item.detail);
      node.querySelector(".rptStepActionType")?.dispatchEvent(new Event("change", { bubbles: true }));
      node.querySelector(".rptAlcoholResult")?.dispatchEvent(new Event("change", { bubbles: true }));
      refreshSingleCardUi(node);
    });
  }

  function fillImages(rows) {
    const listId = "rptImageList";
    const root = $(listId);
    if (!root) return;
    root.innerHTML = "";
    const arr = Array.isArray(rows) && rows.length ? rows : [{}];
    arr.forEach((item, idx) => {
      const node = appendRow(listId, createImageRowHtml(idx + 1), getRepeatConfig(listId).label);
      if (node.querySelector(".rptImageCaption")) node.querySelector(".rptImageCaption").value = norm(item.caption);
      if (item && (item.id || item.url || item.viewUrl)) applyExistingImagePreview(node, item);
      refreshSingleCardUi(node);
    });
  }

  function setEditModeBanner() {
    const card = $("report500Card");
    if (!card) return;
    let banner = $("report500EditBanner");
    if (!banner) {
      banner = document.createElement("div");
      banner.id = "report500EditBanner";
    ...
    banner.innerHTML = `
      <div class="sectionMain">โหมดแก้ไข Report500</div>
      <div class="sectionSub">กำลังแก้ไขเอกสาร Ref ${escapeHtml(state.editMode.sourceRefNo || state.editMode.rootRefNo || "-")}${state.editMode.revisionLabel ? ` • ${escapeHtml(state.editMode.revisionLabel)}` : ""}</div>
      <div class="panelActions">
        <button type="button" id="btnRptManageExistingImages" class="btn ghost">จัดการรูปเดิม</button>
        <button type="button" id="btnRptExitEditMode" class="btn danger">ออกจากโหมดแก้ไข</button>
      </div>
    `;
    $("btnRptManageExistingImages")?.addEventListener("click", openReportExistingImageManager_);
    $("btnRptExitEditMode")?.addEventListener("click", async () => {
      const res = await Swal.fire({
        icon: "question",
  ...
  async function openReportExistingImageManager_() {
    const images = Array.isArray(state.editMode.existingImages) ? state.editMode.existingImages : [];
    await Swal.fire({
      title: "จัดการรูปเดิมของ Report500",
      width: 1100,
      html: images.length ? `
        <div class="galGrid">
          ${images.map((img, idx) => {
            const id = norm(img.id);
            const deleted = state.editMode.removedImageIds.includes(id);
            const url = norm(img.url || img.viewUrl);
            return `
              <div class="galItem">
                <div class="galThumbWrap">${url ? `<img class="galThumb" src="${escapeHtml(url)}" alt="img-${idx + 1}">` : `<div class="galThumb imageGridEmpty">ไม่มีภาพ</div>`}<span class="galBadge">${idx + 1}</span></div>
                <div class="galCap">${escapeHtml(img.caption || img.filename || img.name || `image-${idx + 1}`)}</div>
                <div class="panelActions"><button type="button" class="btn ${deleted ? "secondary" : "danger"}" data-rpt-oldimg="${escapeHtml(id)}">${deleted ? "คงไว้" : "ลบออก"}</button></div>
              </div>
            `;
          }).join("")}
        </div>
      ` : `<div class="emptyBox">ไม่มีรูปเดิม</div>`,
      didOpen: () => {
        document.querySelectorAll("[data-rpt-oldimg]").forEach((btn) => {
          btn.addEventListener("click", () => {
            const id = norm(btn.getAttribute("data-rpt-oldimg"));
            const idx = state.editMode.removedImageIds.indexOf(id);
            if (idx >= 0) state.editMode.removedImageIds.splice(idx, 1);
            else state.editMode.removedImageIds.push(id);
            Swal.close();
            openReportExistingImageManager_();
          });
        });
      },
      confirmButtonText: "ปิด"
  ...
  async function promptLoadReport500ForEdit_() {
    const { value: refNo } = await Swal.fire({
      title: "โหลด Report500 เพื่อแก้ไข",
      input: "text",
      ...
      cancelButtonText: "ยกเลิก",
      showCancelButton: true,
      inputValidator: (value) => (!norm(value) ? "กรุณากรอก Ref" : "")
    });
    if (!refNo) return;
    await loadReport500ForEditByRef_(refNo);
  }

  async function loadReport500ForEditByRef_(refNo) {
    const safeRef = norm(refNo);
    const res = await fetch(`${apiUrl("/report500LoadForEdit")}?refNo=${encodeURIComponent(safeRef)}`, { method: "GET", cache: "no-store" });
    const text = await res.text();
    let json = {};
    try { json = JSON.parse(text); } catch (_) { throw new Error("Backend ตอบกลับไม่ใช่ JSON"); }
    if (!res.ok || !json?.ok) {
    ...
    applyReport500EditPayloadToForm_(json.payload || json);
    await Swal.fire({ icon: "success", title: "โหลดข้อมูลสำเร็จ", text: `ระบบได้โหลดเอกสาร Ref ${safeRef} สำหรับแก้ไขแล้ว` });
  }

  function applyReport500EditPayloadToForm_(payload) {
    const p = payload || {};
    state.editMode.active = true;
    state.editMode.sourceRefNo = norm(p.refNo);
    state.editMode.rootRefNo = norm(p.rootRefNo || p.refNo);
    state.editMode.revisionNo = Number(p.revisionNo || 0);
    state.editMode.revisionLabel = norm(p.revisionLabel);
    state.editMode.editedFromRefNo = norm(p.editedFromRefNo || p.refNo);
    state.editMode.editedAt = norm(p.editedAt);
    state.editMode.editedBy = norm(p.editedBy);
    state.editMode.documentId = norm(p.documentId);
    state.editMode.existingImages = Array.isArray(p.existingImages) ? p.existingImages.map((x) => ({ ...x })) : [];
    state.editMode.removedImageIds = [];

    const refText = norm(p.rootRefNo || p.refNo);
    const refMatch = refText.match(/^(\d+)-(\d{4})$/);
    if (refMatch) {
      if ($("rptRefNo")) $("rptRefNo").value = refMatch[1];
      if ($("rptRefYear")) $("rptRefYear").value = refMatch[2];
    }

    setReadonlyValue("rptReportedBy", norm(getAuth().name || p.reportedBy));
    if ($("rptReporterPosition")) { ensureOptionValue($("rptReporterPosition"), norm(p.reporterPosition)); $("rptReporterPosition").value = norm(p.reporterPosition); }
    if ($("rptReporterPositionOther")) $("rptReporterPositionOther").value = norm(p.reporterPositionOther);
    if ($("rptReportDate")) $("rptReportDate").value = norm(p.reportDateIso || p.reportDate);
    if ($("rptBranch")) { ensureOptionValue($("rptBranch"), norm(p.branch)); $("rptBranch").value = norm(p.branch); }
    if ($("rptBranchOther")) $("rptBranchOther").value = norm(p.branchOther);
    if ($("rptSubject")) $("rptSubject").value = norm(p.subject);
    if ($("rptIncidentDate")) $("rptIncidentDate").value = norm(p.incidentDateIso || p.incidentDate);
    if ($("rptIncidentTime")) $("rptIncidentTime").value = norm(p.incidentTime);
    if ($("rptWhereDidItHappen")) { ensureOptionValue($("rptWhereDidItHappen"), norm(p.whereDidItHappen)); $("rptWhereDidItHappen").value = norm(p.whereDidItHappen); }
    if ($("rptArea")) $("rptArea").value = norm(p.area);
    if ($("rptWhatHappen")) $("rptWhatHappen").value = norm(p.whatHappen);
    if ($("rptOffenderStatement")) $("rptOffenderStatement").value = norm(p.offenderStatement);
    if ($("rptSummaryText")) $("rptSummaryText").value = norm(p.summaryText);
    if ($("rptDisciplineEmployeeCode")) $("rptDisciplineEmployeeCode").value = norm(p.disciplineEmployeeCode);
    if ($("rptDisciplineEmployeeName")) $("rptDisciplineEmployeeName").value = norm(p.disciplineEmployeeName);

    $("rptBranch")?.dispatchEvent(new Event("change", { bubbles: true }));
    $("rptReporterPosition")?.dispatchEvent(new Event("change", { bubbles: true }));

    setChecksByName("rptReportTypes", p.reportTypes || p.reportTypesSelections || []);
    setChecksByName("rptUrgencyTypes", p.urgencyTypes || p.urgencySelections || []);
    setChecksByName("rptNotifyTo", p.notifyTo || p.notifyToSelections || []);
    setWhereTypeSelections(p.whereTypeSelections || p.whereTypes || []);

    fillPersons(p.involvedPersons || []);
    fillIndexedRows("rptDamageList", p.damages || [], "damage", "ความเสียหาย", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillStepTakens(p.stepTakens || []);
    fillIndexedRows("rptEvidenceList", p.evidences || [], "evidence", "หลักฐาน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillIndexedRows("rptCauseList", p.causes || [], "cause", "สาเหตุ", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillIndexedRows("rptPreventionList", p.preventions || [], "prevention", "การป้องกัน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillIndexedRows("rptLearningList", p.learnings || [], "learning", "ข้อสรุป/บทเรียน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillImages(p.existingImages || p.images || []);

    state.itemLookup = {
      item: norm(p.item || ""),
      description: norm(p.itemDescription || ""),
      displayText: norm(p.itemDisplay || ""),
      found: !!norm(p.item || ""),
      searched: !!norm(p.item || "")
    };

    if (Array.isArray(p.disciplineReference) && p.disciplineReference.length) {
      attachDisciplineLookupResult({
        employeeCode: p.disciplineEmployeeCode,
        employeeName: p.disciplineEmployeeName,
        records: p.disciplineReference,
        count: p.disciplineMatchCount || p.disciplineReference.length
      });
    } else {
      resetDisciplineLookupState();
    }

    setEmailSelections(p.emailRecipients || [], p.emailOther || "");
    setEditModeBanner();
  }

  function countSelectedFiles() {
    const inputs = Array.from(document.querySelectorAll('#rptImageList input[type="file"]'));
    return inputs.reduce((acc, el) => {
      const edited = RPT_EDITED_IMAGE_STORE.get(el)?.file || null;
  ...
  function collectPayload() {
    const payload = {
      refNo: getRefNo(),
      reportedBy: norm($("rptReportedBy")?.value || getAuth().name),
      reporterPosition: norm($("rptReporterPosition")?.value),
      reporterPositionOther: norm($("rptReporterPositionOther")?.value),
      reportDate: norm($("rptReportDate")?.value),
      branch: norm($("rptBranch")?.value),
      ...
      preventions: collectIndexedRows("rptPreventionList"),
      learnings: collectIndexedRows("rptLearningList"),
      emailRecipients: collectEmailRecipients(),
      emailOther: norm($("rptEmailOther")?.value),
      disciplineEmployeeCode: norm($("rptDisciplineEmployeeCode")?.value || state.disciplineLookup.employeeCode),
      disciplineEmployeeName: norm($("rptDisciplineEmployeeName")?.value || state.disciplineLookup.employeeName),
      disciplineReference: state.disciplineLookup.attached ? state.disciplineLookup.records : [],
      disciplineMatchCount: state.disciplineLookup.attached ? (state.disciplineLookup.matchCount || state.disciplineLookup.records.length) : 0,
      item: state.itemLookup.item,
      itemDescription: state.itemLookup.description,
      itemDisplay: state.itemLookup.displayText
    };

    if (state.editMode.active) {
      payload.rootRefNo = state.editMode.rootRefNo || payload.refNo;
      payload.revisionNo = Number(state.editMode.revisionNo || 0);
      payload.revisionLabel = norm(state.editMode.revisionLabel);
      payload.editedFromRefNo = norm(state.editMode.editedFromRefNo || state.editMode.sourceRefNo || payload.rootRefNo);
      payload.editedAt = norm(state.editMode.editedAt);
      payload.editedBy = norm(getAuth().name || state.editMode.editedBy);
      payload.documentId = norm(state.editMode.documentId);
      payload.existingImages = (state.editMode.existingImages || []).map((x) => ({ ...x }));
      payload.removedImageIds = (state.editMode.removedImageIds || []).slice();
    }
  ...
  function validatePayload(p) {
    ...
    if (!norm(p.branch)) return "กรุณาเลือกสาขา";
    if (isOther(p.branch) && !norm(p.branchOther)) return "กรุณาระบุสาขาอื่นๆ";
    if (!norm(p.subject)) return "กรุณากรอกเรื่อง";
    if (!Array.isArray(p.reportTypes) || !p.reportTypes.some((x) => x.checked)) return "กรุณาเลือกประเภทรายงานอย่างน้อย 1 รายการ";
    if (!Array.isArray(p.urgencyTypes) || !p.urgencyTypes.some((x) => x.checked)) return "กรุณาเลือกระดับความเร่งด่วนอย่างน้อย 1 รายการ";
    if (!norm(p.incidentDate)) return "กรุณาระบุวันที่เกิดเหตุ";
    if (!norm(p.whereDidItHappen)) return "กรุณาเลือกสถานที่เกิดเหตุหลัก";
    if (!norm(p.whatHappen)) return "กรุณากรอกเหตุที่เกิด";
    if (!Array.isArray(p.involvedPersons) || !p.involvedPersons.length) return "กรุณากรอกผู้เกี่ยวข้องอย่างน้อย 1 รายการ";
    if (!Array.isArray(p.stepTakens) || !p.stepTakens.length) return "กรุณากรอกการดำเนินการอย่างน้อย 1 รายการ";
    return "";
  }

  function payloadSummaryHtml(p) {
    const emails = Array.isArray(p.emailRecipients) ? p.emailRecipients : [];
    const fileCount = countSelectedFiles();
    return `
      <div class="swalSummary">
        <div class="swalHero">
          <div class="swalHeroTitle">สรุปข้อมูลก่อนบันทึก Report</div>
          <div class="swalHeroSub">กรุณาตรวจสอบข้อมูลสำคัญก่อนดำเนินการ</div>
          <div class="swalPillRow">
            <div class="swalPill primary">ผู้รายงาน: ${escapeHtml(p.reportedBy || "-")}</div>
        ...
        <div class="swalSection">
          ...
          <div class="swalKvGrid">
            <div class="swalKv"><div class="swalKvLabel">Ref</div><div class="swalKvValue">${escapeHtml(p.refNo || "-")}</div></div>
            <div class="swalKv"><div class="swalKvLabel">สาขา</div><div class="swalKvValue">${escapeHtml(p.branch || "-")}</div></div>
            <div class="swalKv"><div class="swalKvLabel">เรื่อง</div><div class="swalKvValue">${escapeHtml(p.subject || "-")}</div></div>
            <div class="swalKv"><div class="swalKvLabel">วันที่เกิดเหตุ</div><div class="swalKvValue">${escapeHtml(p.incidentDate || "-")}</div></div>
          </div>
        </div>
      </div>
    `;
  ...
  async function preview() {
    ...
    if (err) {
      return Swal.fire({ icon: "warning", title: "ข้อมูลไม่ครบ", text: err, confirmButtonText: "ตกลง" });
    }
    await Swal.fire({
      title: "ตรวจสอบก่อนบันทึก",
      html: payloadSummaryHtml(p),
      confirmButtonText: "ตกลง",
  ...
  async function submit() {
    ...
    if (!state.editMode.active) {
      try {
        ...
        return Swal.fire({ icon: "error", title: "ตรวจสอบ Ref ไม่สำเร็จ", text: dupErr?.message || String(dupErr), confirmButtonText: "ตกลง" });
      }
    }
    ...
    try {
      ...
      return Swal.fire({ icon: "warning", title: "ตรวจสอบไฟล์ไม่ผ่าน", text: fileErr?.message || String(fileErr) });
    }

    const isEdit = state.editMode.active;
    const action = isEdit ? "report500Revise" : "report500Submit";
    const body = {
      action,
      pass: getAuth().pass,
      payload: p,
    ...
    if (window.ProgressUI) {
      window.ProgressUI.show(isEdit ? "กำลังบันทึกการแก้ไข Report" : "กำลังบันทึก Report", "ระบบกำลังอัปโหลดข้อมูล สร้าง PDF และส่งอีเมล");
      window.ProgressUI.activateOnly("validate", 12, isEdit ? "กำลังตรวจสอบข้อมูลการแก้ไข" : "กำลังตรวจสอบข้อมูล");
    }
    ...
    try {
      if (!isEdit) {
        const dupBeforeSend = await checkReportRefDuplicate_({ force: true });
        if (dupBeforeSend.duplicated) {
          window.ProgressUI?.markError("validate", "เลขอ้างอิงซ้ำ", 12);
          window.ProgressUI?.hide(150);
          await showReportDuplicateAlert_(dupBeforeSend.result);
          $("rptRefNo")?.focus();
          return;
        }
      ...
      window.ProgressUI?.markDone("validate", 22, "พร้อมส่งข้อมูล");
      window.ProgressUI?.activateOnly("upload", 36, "กำลังเตรียมรูปภาพ");
      window.ProgressUI?.markDone("upload", 48, `เตรียมไฟล์เรียบร้อย (${files.length} ไฟล์)`);
      window.ProgressUI?.activateOnly("save", 64, isEdit ? "กำลังบันทึกฉบับแก้ไข" : "กำลังบันทึกข้อมูลลงระบบ");
      ...
      const text = await res.text();
      let json = {};
      try { json = JSON.parse(text); } catch (_) { throw new Error("ระบบตอบกลับไม่ใช่ JSON"); }
      if (!res.ok || !json?.ok) {
        throw new Error(json?.error || json?.message || `บันทึกข้อมูลไม่สำเร็จ (HTTP ${res.status})`);
      }

      window.ProgressUI?.markDone("save", 78, "บันทึกลงระบบเรียบร้อย");
      window.ProgressUI?.activateOnly("pdf", 88, "กำลังสร้าง PDF");
      window.ProgressUI?.markDone("pdf", 94, "สร้าง PDF เรียบร้อย");
      window.ProgressUI?.activateOnly("email", 98, "กำลังตรวจสอบผลการส่งอีเมล");
      window.ProgressUI?.markDone("email", 100, "ดำเนินการเสร็จแล้ว");
      window.ProgressUI?.success(isEdit ? "บันทึกการแก้ไขสำเร็จ" : "บันทึกสำเร็จ", "ระบบสร้างเอกสารเรียบร้อยแล้ว");
      window.ProgressUI?.hide(180);

      await Swal.fire({
        icon: "success",
        title: isEdit ? "บันทึกการแก้ไขสำเร็จ" : "บันทึกสำเร็จ",
        html: `
          <div class="swalSummary">
            <div class="swalHero">
              <div class="swalHeroTitle">${escapeHtml(isEdit ? "บันทึกการแก้ไขสำเร็จ" : "บันทึกสำเร็จ")}</div>
              <div class="swalHeroSub">เลขอ้างอิง ${escapeHtml(json.refNo || p.refNo || "-")}</div>
            </div>
            ${json.pdfUrl ? `<div class="swalActionLink"><a href="${escapeHtml(json.pdfUrl)}" target="_blank" rel="noopener noreferrer">เปิด PDF</a></div>` : ""}
          </div>
        `,
        width: 920,
        confirmButtonText: "ปิด"
      ...
      console.error(err);
      window.ProgressUI?.markError("save", err?.message || String(err), 70);
      window.ProgressUI?.hide(180);
      await Swal.fire({ icon: "error", title: "บันทึกข้อมูลไม่สำเร็จ", text: err?.message || String(err), confirmButtonText: "ตกลง" });
    }
  }
  ...
  function resetForm() {
    ...
    if ($("rptIncidentDate")) $("rptIncidentDate").value = todayIsoLocal();
    if ($("rptIncidentTime")) $("rptIncidentTime").value = "";
    if ($("rptWhereDidItHappen")) $("rptWhereDidItHappen").value = "";
    if ($("rptArea")) $("rptArea").value = "";
    if ($("rptWhatHappen")) $("rptWhatHappen").value = "";
    if ($("rptOffenderStatement")) $("rptOffenderStatement").value = "";
    if ($("rptSummaryText")) $("rptSummaryText").value = "";
    ...
    document.querySelectorAll('input[name="rptReportTypes"], input[name="rptUrgencyTypes"], input[name="rptNotifyTo"], .rptEmailChk, .rptWhereTypeChk').forEach((el) => {
      el.checked = false;
      el.dispatchEvent(new Event("change", { bubbles: true }));
    });
    document.querySelectorAll(".rptWhereTypeSuffix").forEach((el) => { el.value = ""; });

    fillPersons([]);
    fillIndexedRows("rptDamageList", [], "damage", "ความเสียหาย", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillStepTakens([]);
    fillIndexedRows("rptEvidenceList", [], "evidence", "หลักฐาน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillIndexedRows("rptCauseList", [], "cause", "สาเหตุ", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillIndexedRows("rptPreventionList", [], "prevention", "การป้องกัน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillIndexedRows("rptLearningList", [], "learning", "ข้อสรุป/บทเรียน", "รายละเอียด", "ระบุหัวข้อ", "ระบุรายละเอียด");
    fillImages([]);
  ...
  async function ensureReady() {
    if (state.loading) return;
    if (state.ready && state.options) {
      const hasSomeOptions = Array.isArray(state.options.branchList) && state.options.branchList.length > 0 && Array.isArray(state.options.reportTypeList) && state.options.reportTypeList.length > 0;
      if (hasSomeOptions) return;
    }
    ...
    try {
      setRefYear();
      const res = await fetch(apiUrl("/report500/options"), { method: "GET", cache: "no-store" });
      const text = await res.text();
      let json = {};
      ...
      console.error("Report500 ensureReady error:", err);
      await Swal.fire({ icon: "error", title: "โหลดตัวเลือกไม่สำเร็จ", text: err?.message || String(err) });
    } finally {
      state.loading = false;
    }
  }
  ...
  function init(deps = {}) {
    if (deps.getAuth) state.auth = deps.getAuth() || state.auth;
    if (deps.apiUrl && typeof deps.apiUrl === "function") window.apiUrl = deps.apiUrl;
    if (deps.escapeHtml && typeof deps.escapeHtml === "function") window.escapeHtml = deps.escapeHtml;
    if (deps.norm && typeof deps.norm === "function") window.norm = deps.norm;
    return ensureReady();
  }
  ...
  window.Report500App = {
    ...
    reset: resetForm,
    onAuthReady,
    promptLoadForEdit: promptLoadReport500ForEdit_,
    loadForEditByRef: loadReport500ForEditByRef_
  };
})();
