...
(() => {
  ...
  function ensureStyles() {
    if (document.getElementById("imageEditorXStyles")) return;

    const style = document.createElement("style");
    style.id = "imageEditorXStyles";
    style.textContent = `
      .ix-backdrop{
      ...
      .ix-modal.ix-open,.ix-backdrop.ix-open{display:block}
      .ix-panel{
        width:min(1200px,100%); height:min(92vh,900px);
        margin:0 auto; background:#fff; border-radius:18px;
      ...
      .ix-head{
        ...
        gap:12px; padding:14px 16px; border-bottom:1px solid #e2e8f0;
        background:#f8fafc;
      }
      .ix-title{font-size:16px; font-weight:800; color:#0f172a}
      .ix-sub{font-size:12px; color:#64748b; margin-top:2px}
      .ix-close{
      ...
      .ix-group{display:flex; gap:8px; align-items:center; flex-wrap:wrap}
      .ix-field{display:grid; gap:4px}
      .ix-field label{font-size:11px; color:#64748b; font-weight:700}
      .ix-field input[type="text"], .ix-field select{
        height:38px; border:1px solid #cbd5e1; border-radius:10px; padding:0 10px;
        font:inherit; min-width:120px;
      }
      .ix-field input[type="number"]{
        height:38px; width:80px; border:1px solid #cbd5e1; border-radius:10px; padding:0 10px; font:inherit;
      }
      .ix-field input[type="color"]{
        height:38px; width:52px; border:1px solid #cbd5e1; border-radius:10px; padding:4px; background:#fff;
      }
      .ix-btn{
        border:none; background:#e2e8f0; color:#0f172a;
        border-radius:10px; padding:10px 12px; cursor:pointer; font-weight:700;
      }
      .ix-btn.primary{background:#2563eb; color:#fff}
      .ix-btn.warn{background:#f59e0b; color:#111827}
      .ix-btn:disabled{opacity:.55; cursor:not-allowed}
      .ix-canvas-wrap{
        position:relative; overflow:auto; background:#0f172a;
      ...
      .ix-zoom{font-size:12px; color:#475569; font-weight:700}
      @media (max-width: 768px){
        .ix-modal{padding:10px}
        .ix-panel{height:min(95vh,1000px)}
        .ix-toolbar{padding:10px}
        .ix-foot{flex-direction:column; align-items:stretch}
      }
    `;
  ...
  function ensureModal() {
    ...
    backdrop.className = "ix-backdrop";

    const modal = document.createElement("div");
    modal.className = "ix-modal";
    modal.innerHTML = `
      <div class="ix-panel">
        <div class="ix-head">
          <div>
            <div class="ix-title">แก้ไขภาพ</div>
            <div class="ix-sub" id="ixFileName">-</div>
          </div>
          <button type="button" class="ix-close" id="ixCloseTop">ปิด</button>
        </div>
        ...
        <div class="ix-toolbar">
          <div class="ix-group">
            <div class="ix-field">
              ...
              <select id="ixToolSelect">
                <option value="select">เลือก</option>
                <option value="draw">วาดเส้น</option>
                <option value="text">ข้อความ</option>
                <option value="rect">สี่เหลี่ยม</option>
                <option value="circle">วงกลม</option>
                <option value="line">เส้นตรง</option>
              </select>
            </div>

            <div class="ix-field">
              <label>สี</label>
              <input id="ixColor" type="color" value="#dc2626">
            </div>

            <div class="ix-field">
              <label>ความหนา</label>
              <input id="ixWidth" type="number" min="1" max="24" step="1" value="3">
            </div>

            <div class="ix-field">
              <label>ข้อความ</label>
              <input id="ixText" type="text" placeholder="พิมพ์ข้อความ">
            </div>
          </div>

          <div class="ix-group">
            <button type="button" class="ix-btn" id="ixUndo">Undo</button>
            <button type="button" class="ix-btn" id="ixRedo">Redo</button>
            <button type="button" class="ix-btn" id="ixReset">Reset</button>
            <button type="button" class="ix-btn warn" id="ixClear">ลบที่เลือก</button>
          </div>

          <div class="ix-group">
            <button type="button" class="ix-btn" id="ixZoomOut">-</button>
            <button type="button" class="ix-btn" id="ixZoomIn">+</button>
          </div>
        </div>

        <div class="ix-canvas-wrap" id="ixCanvasWrap">
          <div class="ix-canvas-stage">
            <canvas id="ixCanvas"></canvas>
          </div>
        </div>

        <div class="ix-foot">
          <div class="ix-zoom" id="ixZoomText">100%</div>
          <div class="ix-group">
            <button type="button" class="ix-btn" id="ixCancel">ยกเลิก</button>
            <button type="button" class="ix-btn primary" id="ixSave">บันทึกภาพ</button>
          </div>
        </div>
      </div>
    `;
    ...
    state.modal = modal;
    state.panel = modal.querySelector(".ix-panel");
    state.canvasWrap = modal.querySelector("#ixCanvasWrap");
    state.canvasEl = modal.querySelector("#ixCanvas");
    state.saveBtn = modal.querySelector("#ixSave");
    state.cancelBtn = modal.querySelector("#ixCancel");
    state.resetBtn = modal.querySelector("#ixReset");
    state.undoBtn = modal.querySelector("#ixUndo");
    state.redoBtn = modal.querySelector("#ixRedo");
    state.clearBtn = modal.querySelector("#ixClear");
    state.zoomInBtn = modal.querySelector("#ixZoomIn");
    state.zoomOutBtn = modal.querySelector("#ixZoomOut");
    state.zoomText = modal.querySelector("#ixZoomText");
    state.colorInput = modal.querySelector("#ixColor");
    state.widthInput = modal.querySelector("#ixWidth");
    state.textInput = modal.querySelector("#ixText");
    state.fileNameEl = modal.querySelector("#ixFileName");
    state.toolSelect = modal.querySelector("#ixToolSelect");

    backdrop.addEventListener("click", () => cancelEditor());
    modal.querySelector("#ixCloseTop")?.addEventListener("click", () => cancelEditor());
    state.cancelBtn?.addEventListener("click", () => cancelEditor());
    state.saveBtn?.addEventListener("click", () => saveEditor());
    state.resetBtn?.addEventListener("click", () => resetToBaseImage());
    state.undoBtn?.addEventListener("click", () => undo());
    state.redoBtn?.addEventListener("click", () => redo());
    state.clearBtn?.addEventListener("click", () => removeSelected());
    state.zoomInBtn?.addEventListener("click", () => setZoom((state.canvas?.getZoom?.() || 1) + 0.1));
    state.zoomOutBtn?.addEventListener("click", () => setZoom((state.canvas?.getZoom?.() || 1) - 0.1));

    state.colorInput?.addEventListener("input", () => {
      state.drawingColor = state.colorInput.value || "#dc2626";
      syncBrush();
    });

    state.widthInput?.addEventListener("input", () => {
      const n = Math.max(1, Math.min(24, Number(state.widthInput.value || 3) || 3));
      state.drawingWidth = n;
      state.widthInput.value = String(n);
      syncBrush();
    });

    state.toolSelect?.addEventListener("change", () => {
      setActiveTool(state.toolSelect.value || "select");
    });
  ...
  function openModalUi(fileName) {
    ensureModal();
    state.fileNameEl.textContent = fileName || "-";
    state.backdrop.classList.add("ix-open");
    state.modal.classList.add("ix-open");
    document.body.classList.add("progress-lock");
    state.opened = true;
  }

  function closeModalUi() {
    state.backdrop?.classList.remove("ix-open");
    state.modal?.classList.remove("ix-open");
    document.body.classList.remove("progress-lock");
    state.opened = false;
  }
  ...
  function destroyCanvas() {
    if (state.canvas) {
      try { state.canvas.dispose(); } catch (_) {}
    }
    state.canvas = null;
  ...
  function revokeOriginalUrl() {
    if (state.originalObjectUrl) {
      try { URL.revokeObjectURL(state.originalObjectUrl); } catch (_) {}
  ...
  async function blobFromUrl(url) {
    ...
    if (!cleanUrl) {
      throw new Error("ไม่พบ URL ของรูปเดิม");
    }
    ...
    if (!res.ok) {
      throw new Error(`โหลดรูปเดิมไม่สำเร็จ (HTTP ${res.status})`);
    }

    return await res.blob();
  }

  function guessFileExtensionFromType(mimeType) {
  ...
  function buildSafeFilenameFromAsset(assetMeta, mimeType) {
    ...
    const ext = guessFileExtensionFromType(mimeType);
    return `existing-image.${ext}`;
  }

  async function fileFromExistingAsset(assetMeta) {
    ...
    if (!candidateUrl) {
      throw new Error("ไม่พบ previewUrl/url ของรูปเดิม");
    }

    const blob = await blobFromUrl(candidateUrl);
    const mimeType = blob.type || String(asset.mimeType || "").trim() || "image/jpeg";
    const filename = buildSafeFilenameFromAsset(asset, mimeType);
  ...
  function createCanvas(width, height) {
    ...
    state.canvas = new fabric.Canvas(state.canvasEl, {
      preserveObjectStacking: true,
      selection: true
    });
  ...
  function bindCanvasEvents() {
    ...
    state.canvas.on("mouse:down", (opt) => {
      ...
      if (state.activeTool === "text") {
        const textValue = state.textInput?.value || "ข้อความ";
      ...
      if (drawingObject) {
        state.canvas.add(drawingObject);
      }
    });

    state.canvas.on("mouse:move", (opt) => {
      if (!state.canvas || !drawingObject) return;
      const pointer = state.canvas.getPointer(opt.e);
    ...
    state.canvas.on("object:modified", () => commitHistory());
    state.canvas.on("object:added", () => {
      if (!state.isApplyingHistory && !state.baseImage) return;
      updateUndoRedoState();
    });
    state.canvas.on("selection:created", updateUndoRedoState);
    state.canvas.on("selection:updated", updateUndoRedoState);
    state.canvas.on("selection:cleared", updateUndoRedoState);
  }

  function syncBrush() {
    if (!state.canvas) return;
    state.canvas.isDrawingMode = state.activeTool === "draw";
    if (!state.canvas.freeDrawingBrush) return;
    state.canvas.freeDrawingBrush.color = state.drawingColor;
    state.canvas.freeDrawingBrush.width = state.drawingWidth;
  }

  function setActiveTool(tool) {
    state.activeTool = tool || "select";
    if (state.toolSelect) state.toolSelect.value = state.activeTool;
  ...
  function updateUndoRedoState() {
    if (state.undoBtn) state.undoBtn.disabled = !(state.historyIndex > 0);
    if (state.redoBtn) state.redoBtn.disabled = !(state.historyIndex < state.history.length - 1);
    if (state.clearBtn) state.clearBtn.disabled = !state.canvas?.getActiveObject?.();
  }
  ...
  function commitHistory() {
    if (!state.canvas || state.isApplyingHistory) return;
  ...
  function applyHistory(serialized) {
    if (!state.canvas || !serialized) return;

    state.isApplyingHistory = true;
    state.canvas.loadFromJSON(serialized, () => {
      state.isApplyingHistory = false;
      state.canvas.renderAll();
      updateUndoRedoState();
      setActiveTool("select");
  ...
  function setZoom(value) {
    if (!state.canvas) return;
    const zoom = Math.max(0.2, Math.min(4, Number(value || 1) || 1));
    state.canvas.setZoom(zoom);
    state.canvas.requestRenderAll();
    if (state.zoomText) state.zoomText.textContent = `${Math.round(zoom * 100)}%`;
  }

  function fitImageToViewport(width, height) {
    const wrap = state.canvasWrap;
    if (!wrap) return 1;

    const maxW = Math.max(320, wrap.clientWidth - 40);
    const maxH = Math.max(240, wrap.clientHeight - 40);
    return Math.min(maxW / width, maxH / height, 1);
  }

  function loadBaseImageFromUrl(url, fileName) {
    return new Promise((resolve, reject) => {
      fabric.Image.fromURL(url, (img) => {
        ...
        const fit = fitImageToViewport(width, height);
        setZoom(fit);
  ...
  async function open(file, options = {}) {
    ...
    state.drawingColor = options.strokeColor || "#dc2626";
    state.drawingWidth = Number(options.strokeWidth || 3) || 3;

    if (state.colorInput) state.colorInput.value = state.drawingColor;
    if (state.widthInput) state.widthInput.value = String(state.drawingWidth);
    if (state.textInput) state.textInput.value = "";
    if (state.toolSelect) state.toolSelect.value = "select";

    openModalUi(file.name || "image");
    await loadBaseImageFromUrl(state.originalObjectUrl, file.name || "image");
    setActiveTool("select");
  ...
  async function openFromExistingAsset(assetMeta, options = {}) {
    const asset = assetMeta || {};

    if (!asset.previewUrl && !asset.url && !asset.src) {
      throw new Error("ยังไม่พบข้อมูลรูปเดิมที่ใช้เปิด editor");
    }

    const file = await fileFromExistingAsset(asset);
    return await open(file, options || {});
  }

  function resetToBaseImage() {
    if (!state.originalObjectUrl || !state.originalFile) return;
    loadBaseImageFromUrl(state.originalObjectUrl, state.originalFile.name || "image")
  ...
  function canvasToFile() {
    return new Promise((resolve, reject) => {
      if (!state.canvas) {
        reject(new Error("ยังไม่มี canvas"));
        return;
      }
      ...
      fetch(dataUrl)
        .then((r) => r.blob())
        .then((blob) => {
          const nameBase = String(state.originalFile?.name || "edited-image").replace(/\.[^.]+$/, "");
          const file = new File([blob], `${nameBase}.png`, {
            type: "image/png",
            lastModified: Date.now()
          });
  ...
  async function saveEditor() {
    try {
      ...
      console.error(err);
      alert(err?.message || "บันทึกภาพไม่สำเร็จ");
    }
  }
